...
def make_slide05():
    ...
    draw.text((W//2, 1322), "BEBA SE NIKADA, NI NA SEKUND, NE OSTAVLJA SAMA!", fill=C_LINEN, font=F_CARD_TITLE, anchor="mm")
...
def make_slide21():
    ...
    draw.text((W//2, 1318), "Beba <3 meseca sa T >38°C, febrilne konvulzije ili letargija!", fill=C_LINEN, font=F_CARD_TITLE, anchor="mm")
...
def make_slide25():
    ...
    # Bottom warning
    draw.rounded_rectangle([(35, 1225), (W-35, 1465)], radius=12, fill=C_DARK)
    draw.text((W//2, 1262), "[ZLATNO PRAVILO ROYAL NANNY DADILJE]", fill=C_GOLD, font=F_BOLD, anchor="mm")
    
    ban_text = "PROSTOR SE PRILAGOĐAVA DETETU PRE NEGO ŠTO SE DESI INCIDENT!"
    b_box = draw.textbbox((0, 0), ban_text, font=F_BOLD)
    f_ban = ImageFont.truetype("/System/Library/Fonts/Supplemental/Arial Bold.ttf", 26) if (b_box[2]-b_box[0] > 1050) else F_BOLD
    draw.text((W//2, 1320), ban_text, fill=C_LINEN, font=f_ban, anchor="mm")
    draw.text((W//2, 1380), "Stalno skeniranje prostora: uvek budite tri koraka ispred detetove radoznalosti.", fill=C_SAND, font=F_BODY, anchor="mm")
    
    img.save(os.path.join(ASSETS_DIR, "slide25_safe_play_hd.png"), "PNG", optimize=True)
...
def make_slide27():
    ...
    draw.text((W//2, 1338), "NIKADA NE STAVLJATI PRSTE 'NA SLEPO' U USTA BEBE!", fill=C_LINEN, font=F_CARD_TITLE, anchor="mm")
...
if __name__ == "__main__":
    ...
    make_slide30()
    make_slide31()
    print("Sve infografike uspešno generisane sa uveličanim segmentima i optimizacijom!")
